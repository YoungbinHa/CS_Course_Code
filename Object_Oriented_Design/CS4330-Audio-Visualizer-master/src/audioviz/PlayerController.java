...
public class PlayerController implements Initializable {
    ...
    private DecimalFormat displayOneDigit;
    private Boolean swich=true;
    
    @Override
    public void initialize(URL url, ResourceBundle rb) {
        displayOneDigit=new DecimalFormat(".0");
    ...
    private void handleReady() {
        ...
        lengthText.setText(displayOneDigit.format(duration.toMillis()));
    ...
    private void handleUpdate(double timestamp, double duration, float[] magnitudes, float[] phases) {
        ...
        currentText.setText(displayOneDigit.format(ms));
        if(swich)
    ...
    // to avoid the sider button go back to normal
    @FXML
    private void handleSliderClicked(MouseEvent event) {
        if (mediaPlayer != null) {
            swich = false;
        }
    }
    
    //when relased the mediaplayer should be changed to new value
    
    @FXML
    private void handleSliderReleased(MouseEvent event) {
        if (mediaPlayer != null) {
            Duration ct = new Duration(timeSlider.getValue());
            mediaPlayer.seek(ct);
        }
        
        swich = true; // go back to normally display the bar
    }
}
